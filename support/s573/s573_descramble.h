...
/* Nonzero while this descrambler still has stream left to hand out.
 *
 * NOT the game-visible "streaming" bit. get_fpga_ctrl (DIO 0xae) bit 12 is
 * `fpga_ctrl[14] && cur >= mp3_start && cur < mp3_end` and is owned by the
 * FABRIC (rtl/k573_mp3stream.v), which advances its own cur from the
 * consumption credit we report back. The game's START/STOP routines are
 * idempotence-guarded on that bit, so synthesizing it here -- with no enable
 * term and no lower bound -- would answer a question the HPS has no business
 * answering. Use this only to decide whether to keep pulling bytes. */
int s573_desc_has_data(const s573_desc_t *s);
